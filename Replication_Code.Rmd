...
dat_dum_train <- dat_dum[ trainIndex,]
dat_dum_val   <- dat_dum[-trainIndex,]
...
identical(dat_dum_val$acc, dat_val$acc)
```
...
#mean((predict(refit_lasso_lm_model, newdata = dat_dum_train) - dat_train$acc)^2)
...
<!-- This is a commented out Rmd cell 
...
xgb_model_tuned <- xgboost(data = train_matrix, 
                           ...
                           params = xgb_params,
                           nrounds = 350,  # Specify nrounds directly
...
dat_train_X_matrix = scale(as.matrix(dat_dum_train[, !names(dat_dum_train) %in% c('acc')]))

dat_val_X_matrix <- scale((as.matrix(dat_dum_val[, !names(dat_dum_val) %in% c('acc')])),
center = attr(dat_train_X_matrix, "scaled:center"), 
scale = attr(dat_train_X_matrix, "scaled:scale"))

dat_train_X_matrix <- cbind(Intercept = 1, dat_train_X_matrix)
dat_val_X_matrix <- cbind(Intercept = 1, dat_val_X_matrix)

# Fit Poisson regression model with L1 regularization 
alpha <- 1
cvfit <- cv.glmnet(x = dat_train_X_matrix, y = dat_dum_train$acc, family = "poisson", alpha = alpha)

# Select the best lambda based on cross-validation
best_lambda <- cvfit$lambda.min

# Refit the model with the selected lambda
fit <- glmnet(x = dat_train_X_matrix, y = dat_dum_train$acc, family = "poisson", alpha = alpha, lambda = best_lambda)

predictions <-predict(fit, newx = dat_val_X_matrix, type = 'response')

# Calculate mean squared error
mse_glm_poi_l1 <- mean((predictions - dat_dum_val$acc)^2)

# Print the mean squared error
print(paste("Mean Squared Error (MSE) for GLM L1:", mse_glm_poi_l1))

# Store the model performance in dataset
model_performance <- rbind(model_performance, list(Model_Name = "GLM Poisson L1", MSE = mse_glm_poi_l1))
...
#selected_data = cbind(selected_data, 'acc' = dat_train$acc)
#selected_data = as.data.frame(selected_data)

# Fit a Poisson regression model without regularization using selected variables
final_glm_model = glmnet(x = selected_data, y = dat_train$acc, family = "poisson", alpha = 0, lambda = 0)

predictions <- predict(final_glm_model, newx = dat_val_X_matrix[, selected_variables], type = "response")

# Calculate mean squared error
mse_poi_l1_refit <- mean((predictions - dat_val$acc)^2)

# Print the mean squared error
print(paste("Mean Squared Error (MSE) on validation set:", mse_poi_l1_refit))

# Store the model performance in dataset
model_performance <- rbind(model_performance, list(Model_Name = "GLM_Poisson_L1 + Refit", MSE = mse_poi_l1_refit))
```
...
```{r}
alpha <- 0
cvfit <- cv.glmnet(x = selected_data, y = dat_train$acc, family = "poisson", alpha = alpha)
...
# Store the model performance in dataset
model_performance <- rbind(model_performance, list(Model_Name = "GLM_Poisson_L1+L2", MSE = mse))
...
```{r}
alpha <- 1
cvfit <- cv.glmnet(x = dat_train_X_matrix, y = dat_train$acc, family = "poisson", alpha = alpha, relax= TRUE)

plot(cvfit, se.bands = FALSE)

print(cvfit)

predictions= predict(cvfit, newx = dat_val_X_matrix, s = 'lambda.1se', gamma = 'gamma.1se')

predictions_NN <- pmax(predictions, 0)

mse <- mean((predictions_NN - dat_val$acc)^2)

print(paste("Mean Squared Error (MSE) on validation set:", mse))

# Store the model performance in dataset
model_performance <- rbind(model_performance, list(Model_Name = "relaxed lasso", MSE = mse))
...
mse_glm_boost <- mean((predictions - dat_val$acc)^2)

# Print the mean squared error
print(paste("Mean Squared Error (MSE) on validation set:", mse_glm_boost))

# Store the model performance in dataset
model_performance <- rbind(model_performance, list(Model_Name = "Poisson_BOOST", MSE = mse_glm_boost))
...
```{r}
# Show the performance of the best model on the training set
print(mean((predict(xgb_model_tuned, newdata = train_matrix) - dat_dum_train$acc)^2))
```

We see the effect of selection bias as well as the fact that our model is not overfitting the training dataset, since the validation error is lower than the training error. 
...
```{r}
# Define the matrix with the full data without acc or int_no
full_matrix <- as.matrix(dat_dum[, !(names(dat_dum) %in% c("acc", "int_no"))])

# Train XGBoost model with tuned parameters
refit_xgb_model_tuned <- xgboost(data = full_matrix, 
                           label = dat_dum$acc,
                           params = xgb_params,
...
```{r}
# Calculate the spearman correlation between 'acc' and 'predicted_acc'
spearman_rank_cor <- cor(final_df$acc, final_df$predicted_acc, method = "spearman")
print(spearman_rank_cor)
```

This suggests a strong positive monotonic relationship between accidents and predicted accidents. Thus, ranking by accident or dangerousness would be similar.

This is what we expect.

```{r}
ggplot(final_df, aes(x = acc, y = predicted_acc)) +
  geom_point() +
  labs(x = "Number of Accidents", y = "Predicted Number of Accidents") +
  ggtitle("Scatter Plot of Number of Accidents vs Predicted Number of Accidents")
