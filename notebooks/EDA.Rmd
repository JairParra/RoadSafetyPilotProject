...
num_vars <- c("int_no", "fi", "fli", "fri", "fti", "cli",
...
std_num_dat <- f_standardize_data(numeric_dat)
...
cor_matrix <- cor(std_num_dat, use = "complete.obs")
...
dat_plt <- dat %>% filter(!is.na(date_plot))  # Exclude rows with NA in date_plot

# Extract month part from the date (ignoring the year)
dat_plt$month_only <- format(dat_plt$date_plot, "%m")
...
# Bar Chart of Average Accidents by Month
ggplot(average_monthly_accidents, aes(x = month_only, y = average_accidents)) +
...
top_intersections <- head(intersection_accidents, 20)

# Plotting the top 20 intersections
ggplot(top_intersections, aes(x = reorder(paste(rue_1, rue_2), total_accidents), y = total_accidents)) +
  geom_bar(stat = "identity", fill = "blue") +
  ggtitle("Top 20 Intersections with most accidents between ") +
  xlab("Intersection") +
  ylab("Number of Accidents") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
