...
numerical_dat <- dat[, sapply(dat, is.numeric)]
...
significant_correlations <- combined_correlations[abs(combined_correlations$spearman) >= 0.25, ]
...
## Stepwise Variables Modelling

```{r}
# Test performance on validation set
predictions <- predict(final_model, newdata = dat_val)
actual <- dat_val$acc
...
# Store the performance in dataset
model_performance <- rbind(model_performance, list(Model_Name = "Stepwise OLS", MSE = mse))

```

## Lasso Variables Modelling 
